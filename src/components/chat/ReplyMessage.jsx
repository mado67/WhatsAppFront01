...
export default function ReplyMessage({ selectedReplyMessage }) {
    const { clearSelection, selectionMode } = useChatUI();
    return (
        <div className={`absolute bottom-14 left-0  bg-[var(--bg-primary)] w-full h-20 p-2 rounded-md pb-0 border-l-4 border-l-green-500 transform transition-transform duration-300 ease-in-out
  ${selectionMode == 'reply' ? "translate-y-0" : "translate-y-[220px]"}`}>
            <button className="absolute top-4 right-4 text-gray-400 cursor-pointer hover:text-white" onClick={clearSelection}>
                <X size={20} />
            </button>
            <div className="bg-[var(--bg-secondary1)] w-full h-full rounded-md ">
