...
export default function ChatOptions({ onClose, option }) {
    ...
    return (
        <div ref={menuRef} className="fixed inset-0 bg-[var(--bg-secondary)]/60 backdrop-blur-sm flex items-center justify-center z-50">
            {option === "shareMessage" && (
                <div className="bg-[var(--bg-secondary)] w-[90%] max-w-md rounded-2xl shadow-xl p-6 relative animate-fadeIn">

                    {/* Header */}
                    <div className="flex items-center justify-between mb-6 ">
                        <h2 className="text-lg font-semibold text-[var(--text-secondary)]">Share Message</h2>
                        <button onClick={onClose} className="text-gray-400 hover:text-[var(--text-secondary)] cursor-pointer">
                            <X />
                        </button>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        {/* Name */}
                        <div>
                            <label className="text-sm text-[var(--text-secondary)] block mb-2 text-left">Message</label>
                            <textarea
                                value={message}
                                onChange={(e) => setMessage(e.target.value)}
                                required
                                className="w-full bg-[var(--bg-primary)] border border-[var(--border-color)] rounded-lg px-4 py-2 text-sm focus:outline-none focus:border-[#00a884]"
            ...
            {option === "deleteChats" && (
                <div className="fixed inset-0 bg-[var(--bg-secondary)]/60 flex items-center justify-center z-50" >
                    <div className="bg-[var(--bg-secondary)] p-6 rounded-lg w-[90%] max-w-md flex flex-col gap-12" ref={menuRef}>
